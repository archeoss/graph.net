...
def load_data():
    # i = 0
    # for line in csv.reader(open("data.csv")):
    #     # print(line)
    #     i += 1
    #     try:
    #         a = {int(str(line[0]).replace('[', '').replace(']', '')) : [int(str(i).replace('[', '').replace(']', '')) for i in line[1:]]}
    #     except:
    #         print(line)
    return {int(str(line[0]).replace('[', '').replace(']', '')) : [int(str(i).replace('[', '').replace(']', '')) for i in line[1:]] for line in csv.reader(open("data.csv"))}
...
g = DataToGraphDict(load_data(), THRESHOLD)
#print(len(g))
#g = preprocess(g, 30)
print(len(g))
#exit(1)
...
labelsData = main(g, CLUSTERS_NUM)
