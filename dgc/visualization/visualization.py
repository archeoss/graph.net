...
def t_sne(embeds, labels, adj, sample_num=2000, show_fig=True):
    """
    ...
    fig, ax = plt.subplots()
    #fig = plt.figure()
    # Скорее всего он рисует связи неправильно
    #for i in range(len(sample_index)):
    #    for j in range(len(sample_index)):
    #        index_a, index_b = sample_index[i], sample_index[j]
    #        if adj[index_a, index_b]: #and not (ts_embeds[i] - mean < 3 * std).all():
    #           x = norm_ts_embeds[i, 0], norm_ts_embeds[j, 0]
    #            y = norm_ts_embeds[i, 1], norm_ts_embeds[j, 1]
    #            plt.plot(x, y, color=plt.cm.Set1(sample_labels[i] % 8))

    for i in range(norm_ts_embeds.shape[0]):
        plt.text(norm_ts_embeds[i, 0], norm_ts_embeds[i, 1], str(sample_labels[i]),
                 color=plt.cm.Set1(sample_labels[i] % 8),
                fontdict={'weight': 'bold', 'size': 7})
        #print(norm_ts_embeds[i])
        ax.add_patch(plt.Circle((norm_ts_embeds[i, 0], norm_ts_embeds[i, 1]), 0.0025, color=plt.cm.Set1((sample_labels[i] + 1) % 8)))
    plt.xticks([])
    plt.yticks([])
